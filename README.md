...
* [`Epoch.json`](Epoch.json) - contains metadata that is important for the interpretation of parameters derived from data in a CDF with a time `DataType`.
...
* [`ResourceID.json`](ResourceID.json) - is a list of SPASE `ResourceID`/CDAWeb dataset ID pairs. There are many CDAWeb dataset IDs without SPASE `ResourceID`s. Also, there is an inconsistency in the naming (e.g., `SWEPAM` data products are under `spase://VSPO` and `spase//NASA`).
